 ...
 * 클래스(class)의 구성요소
 * 1. 맴버필드 (클래스변수)
 *   - 접근지정자 (public, protected, default, private) => 캡술화(정보은닉)
 * 2. 생성자 메서드
 *   - 클래스명과 동일하게 작성한다.
 *   - 객체를 생성할때 호출된다. new <- 메서드가 호출됨
 *   - 맴버필드를 초기화할 목적으로 사용한다.
 *   - 결과형 리턴값을 담지 않는다 (void 조차 쓰지 않는다)
 *   - 오버로딩(overloading): 동일한 이름을 가진 메서드를 여러개 만들어서 사용할수 있는 기능
 *     단, 매개변수의 갯수가 다르거나, 매개변수의 자료형이 다른경우 사용 가능
 *   - this, this()  
 * 3. 맴버메서드
 *   - 접근지정자: public
 *   - set, get
 *   - 형식
 *     접근지정자(접근제한자) [지정예약어] 리턴자료형 메서드명(매개변수) [throws 예외클래스] {
 *       내용
 *     }
 *   - 유형 4가지
 *     결과형리턴 매개변수

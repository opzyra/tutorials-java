 ...
 * <변수명의 표기법>
 * 스네이크 케이스(snake case)
 * => 언더바(_)가 들어있는 표현 방식 뱀처럼 생겼다고 하여 스네이크 케이스라고 한다.
 * int snake_case;
 * 데이터베이스의 필드명
 * 
 * 카멜 케이스(camel case)
 * => 중간 글자들은 대문자로 시작하지만 첫글자가 소문자인 경우 낙타와 모양이 비슷하다하여 카멜 케이스라고 한다.
 * String camelCase;
 * 자바의 변수에 활용
 * 
 * 파스칼 케이스(pascal case)
 * => 첫글자와 중간글자들이 대문자인 경우 파스칼 언어의 표기법과 유사하여 파스칼 케이스라고함.
 * float PascalCase;
 * 주로 자바의 클래스에 활용!
 * 
 * */

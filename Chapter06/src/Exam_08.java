 ...
 *   BuffedredReader in = new BuffredReader(new InputStreamReader(System.in));
 *                   인스턴스 => 저 BuffedredReader 접근하기 위한 열쇠 키
 * 4) 객체를 통해 맴버에 접근하는 방법: direct연산자
 *   객체명.맴버필드 or 객체명.맴버메서드();
 *   in.readLine();
 * */

/*
 * 접근제한자(접근지정자) 결과형리턴값 메서드명(매겨변수들) [throws 예외 클래스들...] {
 *   내용 정의부;
 * }
 * */
...
class Person {
	 ...
	 * 생성자 메서드
	 * 객체가 생성될때 호출되는 메서드
	 * 생성자 메서드가 존재하지 않으면 default 생성자가 생략되어 있는걸로 간주한다.
	 ...
	 * 생성자도 오버로딩이 가능하다. => 동일한 이름을 가진 메서드를 여러개 만들어서 사용하는기술
	 * 단, 매개변수의 개수가 다르거나, 데이터 타입(자료형)이 달라야한다.
	 * 
	 * this: 클래스 내에서 자기 자신의 객체를 가리킨다.
	 * this(): 클래스 내의 자기 잔신의 또다른 생성자 
	 * */
	
	// 생성자 메서드는 
	// 1. 결과형리턴값이 없다!
	// 2. 클래스이름과 메서드 이름이 동일하다!
	// 3. 작성하지 않으면 생략되어 있다.
	...
	public Person(String name) { // 생성자
		...
		// 메서드 안에서 클래스변수, 맴버필드에 접근할때는 this라는 키워드를 사용한다.
		this.name = name;
	}
	...
	// 맴버 매서드: 객체가 하는 행위나 동작등을 기술하는 영역

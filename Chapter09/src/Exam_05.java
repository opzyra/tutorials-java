/*
 * String 클래스의 유용한 메서드
 * charAt => 지정된 index의 문자를 char타입으로 리턴
 * contains => 문자열이 포함되어있는지 검사한다.
 * equals => 문자열이 일치하는지 체크한다. (대소문자 구분)
 * equalsIgnoreCase => 대소문자를 구분하지 않고 문자열이 일치하는지 체크한다.
 * indexOf => 주어진 문자가 문자열에 존재하는지 체크하여 위치를 알려주고 없다면 -1을 리턴한다.
 * length => 문자열의 길이를 리턴해준다.
 * replace => 문자열 중의 문자(정규표현식에 일치)를 새로운 문자로 변환한다.
 * split => 문자열을 지정된 분리자로 나누어 문자열 배열에 담아 변환한다.
 * substring => 주어진 시작 위치 부터 끝 위치 범위에 포함된 문자열을 추출한다.
...
public class Exam_05 {
	public static void main(String[] args) {
		...
		System.out.println(abc.toUpperCase()); // ABCDEFGHIJK
		System.out.println(ABC.toLowerCase()); // abcdefghijk

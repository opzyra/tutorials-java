		...
		while(true) {
			...
			case 1:
				...
				if(createMenu == 1) {
					...
					int salesResut = Integer.parseInt(in.readLine());
					
					System.out.print("보너스율 = ");
					float rate = Float.parseFloat(in.readLine());
					
					service.createEmployee(name, salary, salesResut, rate);
					System.out.println(name + "직원이 등록되었습니다.");
			...
			case 2:
				...
				if(searchMenu == 1) {
					...
					System.out.println("정규직 = " + dto.getPermanentCount());
					System.out.println("영업직 = " + dto.getSalesCount());
					...
					for(int i = 0; i < empList.length; i++) {
						if(empList[i] != null) {
							...
							System.out.print(empList[i].getSalary() + "\n");
						}
					}
					...
					if(employee == null) {
						...
						if(employee instanceof Permanent) {
							...
							System.out.println("판매실적:" + sales.getSalesResut());
			...
			case 3:
				...
				if(employee == null) {
					...
					if(updateMenu == 1) {
						...
						service.updateEmployee(updateIdx, newName, salary, salesResut, rate);
						System.out.println(newName + "직원이 수정되었습니다.");
